...
function happy_cafe_scripts() {
   ...
   --------------------------------------------------------------*/
    ...
    wp_enqueue_style( 'customstyle', get_template_directory_uri() . '/css/page_whatwedostyle.css');
...
// register footer sidebar with a function

function whatwedo_sidebar() {

    register_sidebar(array(
        'name'          => 'What We Do Sidebar',  // Widget area title
        'id'            => 'sidebar-whatwedo',  // system name
        'description'   => 'Horizontal Sidebar in the What We Do Page', // desc
        'before_widget' => '<div id="%1$s" class="%2$s col-md-6">',  // markup that appears before each widget
        'after_widget'  => '</div>', // 
...
add_action( 'widgets_init', 'whatwedo_sidebar' );



/* Register single menu */
function whatwedo_menu() {
  register_nav_menu('whatwedo-menu',__( 'Header Menu' ));
}
add_action( 'init', 'whatwedo_menu' );

/* short codes*/

function myFunction(){

    $text = 'line 1 my shortcode is working';
    $text .= '<br>';
    $text .= 'line 2, yes it is working';
    $text .= '<br>';
    $text .= '<br>';
...
/* short codes with attribute */

function user_info( $atts = '') {
    ...
    return 'This is ' . $value['name'] . ', he is ' . $value['age'] . ' years old.'. '<br>' .
 'User number: ' . $value['usernum'] . 'with group number' . $value['group'];

}

add_shortcode('user_info', 'user_info');

/* shortcodes with image*/
...
add_shortcode('placeholder_image', 'dynamic_placeholder_image');
...
/****************************************** shortcode bootstrap column example  **********************************/
